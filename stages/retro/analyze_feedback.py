...
parsed = [f["parsed"] for f in feedback_items if f.get("is_parsed_yaml") and f.get("parsed")]

if not parsed:
    # No feedback to analyze
    json.dump({
        "themes": ["No agent feedback was collected during this sprint"],
        "improvements": ["Ensure agents post caloron_feedback comments on task completion"],
        "learnings": [],
        "sentiment": "neutral",
    }, sys.stdout)
    sys.exit(0)
...
if low_clarity:
    ...
    improvements.append("Add more detail to task specifications — include expected inputs/outputs and acceptance criteria")
...
if all_blockers:
    themes.append(f"{len(all_blockers)} blockers reported across {len(parsed)} tasks")
    # Group common blocker patterns
    dep_blockers = [b for b in all_blockers if "depend" in b.lower() or "dag" in b.lower()]
    tool_blockers = [b for b in all_blockers if "tool" in b.lower() or "unavailable" in b.lower()]
    if dep_blockers:
        improvements.append(f"Add missing DAG dependencies — {len(dep_blockers)} runtime deps discovered")
    if tool_blockers:
        improvements.append(f"Add missing tools to agent definitions — {len(tool_blockers)} tool gaps found")

# Theme 3: Assessment distribution
completed = sum(1 for p in parsed if p.get("self_assessment") == "completed")
failed = sum(1 for p in parsed if p.get("self_assessment") in ("failed", "crashed"))
if failed > 0:
    themes.append(f"{failed} tasks failed or crashed out of {len(parsed)}")
    improvements.append("Investigate failed tasks — consider stronger models or clearer specifications")
...
if tokens:
    ...
    if high:
        themes.append(f"{len(high)} tasks used significantly more tokens than average ({avg:.0f})")

# Learnings
if kpis.get("completion_rate", 0) >= 0.8:
    learnings.append("Sprint had a healthy completion rate — task scoping was appropriate")
if kpis.get("avg_interventions", 0) > 1:
    learnings.append("High intervention rate — agents may need better prompts or more context")
if completed == len(parsed):
    learnings.append("All tasks completed successfully — ready to increase sprint scope")
